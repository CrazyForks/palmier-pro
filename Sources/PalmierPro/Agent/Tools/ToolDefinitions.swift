...
enum ToolName: String, CaseIterable, Sendable {
    ...
    case rippleTrimClip = "ripple_trim_clip"
...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            name: .rippleTrimClip,
            description: "Lengthens or shortens ONE clip at its head or tail and RIPPLES: everything after the trimmed edge on that track — plus every sync-locked track — slides by the same amount, so nothing is overwritten and no gap is left behind. This is shift-dragging a clip's edge in the timeline, and it's the tool for \"that shot ends too early / runs too long\": extending reveals more of the source past the current in/out point (its handles), shortening gives those frames back.\n\nPick the right tool for the shape of the edit: ripple_trim_clip changes ONE clip's length at an edge and moves everything downstream; ripple_delete_ranges cuts spans out of the MIDDLE of clips (use it for filler words, dead air, and any multi-range cut); set_clip_properties trims (trimStartFrame/trimEndFrame/durationFrames) resize a clip IN PLACE, leaving a gap or overwriting a neighbor; insert_clips/add_clips bring in new material.\n\nLength, exactly one of:\n• deltaFrames — signed, edge-relative: POSITIVE always lengthens the clip at the named edge, negative shortens it. Downstream clips shift right for a positive value, left for a negative one.\n• toFrame — absolute project frame the edge lands on. For 'tail' it's the clip's new end (exclusive, the same [start, end) frames get_timeline and get_transcript report), so \"keep the last word, which ends at frame 659\" is toFrame 660. For 'head' a ripple keeps startFrame ANCHORED (the frames come off the front of the source and the clip's tail plus everything downstream slides left), so toFrame names the project frame whose content becomes the clip's new first frame and must sit inside the clip — extend a head with a positive deltaFrames instead. An edge already at the requested frame reports changed:false and makes no edit.\n\nLinked audio/video partners trim together, and multicam clips that share the trimmed boundary trim as one group — this is the timing edit multicam accepts, where set_clip_properties trims are refused. Refused without changing anything when the ripple would cut through the middle of a multicam clip (split its clips at that frame first, or cut time with ripple_delete_ranges).\n\nExtends are capped by the source handles left on every clip trimmed together; shrinks are capped by one-frame minimum length and by sync-locked tracks with no room to slide left. The receipt always reports requestedDeltaFrames and appliedDeltaFrames, and names the cap in notes when they differ — read it instead of assuming the full amount landed.",
            inputSchema: objectSchema(
                properties: [
                    "clipId": ["type": "string", "description": "Clip to trim. Its linked partner and any multicam group member sharing the trimmed boundary are trimmed with it."],
                    "edge": [
                        "type": "string",
                        "enum": ["head", "tail"],
                        "description": "'tail' = the clip's out point (its end frame); 'head' = its in point (the source frames at the front, with startFrame anchored).",
                    ],
                    "deltaFrames": ["type": "integer", "description": "Signed length change in project frames: positive lengthens the clip at that edge, negative shortens it. Mutually exclusive with toFrame."],
                    "toFrame": ["type": "integer", "description": "Project frame the edge lands on. Tail: the clip's new end (exclusive). Head: the project frame that becomes the clip's first frame (must be inside the clip). Mutually exclusive with deltaFrames."],
                ],
